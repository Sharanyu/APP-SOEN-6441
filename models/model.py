...
class DBController:
    ...
    def view_emp(self):
        print("Model : Query Received from Controller")
        self._db_connection()
        self.cur.execute("select employee.employee_id,employee.employee_name,fiscal_year,work_location_borough,gross_salary_USD from employee INNER JOIN employee_details ON employee.employee_id=employee_details.employee_id limit 200")
        print("Model: Query Executed. Sending Results back to Controller")
        return self.cur.fetchall()
    ...
    def view_emp_from(self, location):
        ...
        self.cur.execute(
            'select employee.employee_id,employee_name,fiscal_year from employee join employee_details on employee.employee_id=employee_details.employee_id where work_location_borough="{0}"'.format(form_query))
        print("Model: Query Executed. Sending Results back to Controller")
        return self.cur.fetchall()
